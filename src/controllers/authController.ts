...
const REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI || 'http://localhost:3000/login';
...
export const authController = new Elysia()
  ...
  .post('/api/auth/google/callback', async ({ body, set }) => {
    try {
      ...
      console.log('Google OAuth callback started with code:', code.substring(0, 20) + '...');
      ...
      if (tokenData.error) {
        ...
        return { 
          success: false, 
          message: `Google OAuth error: ${tokenData.error_description || tokenData.error}`,
      ...
      if (!user) {
        ...
        return { 
          success: false, 
          message: 'Akun Anda belum terdaftar dalam sistem. Silakan hubungi administrator untuk mendaftarkan akun Anda.',
  ...
  .post('/api/auth/admin/add-user', requireAuth(async ({ body, set, user }) => {
    try {
