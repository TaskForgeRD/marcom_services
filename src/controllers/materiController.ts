...
export const materiController = new Elysia()
  ...
  .put('/api/materi/:id', requireAuth(async ({ params: { id }, request, user, set }) => {
    try {
      const formData = await request.formData();
      const result = await materiService.updateMateri(parseInt(id), formData, user.userId);
  ...
  .delete('/api/materi/:id', requireAuth(async ({ params: { id }, user, set }) => {
    try {
      const result = await materiService.deleteMateri(parseInt(id), user.userId);
...
export const statsController = new Elysia()
  .get('/api/stats', requireAuth(async ({ user }) => {
    try {
