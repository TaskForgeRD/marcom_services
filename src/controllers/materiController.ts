// controllers/materiController.ts - Updated with Socket.IO integration
...
export const materiController = new Elysia()
  ...
  .get(
    "/api/materi/:id",
    requireAuth(async ({ params: { id }, user, set }) => {
      const materi = await materiService.getMateriById(
        parseInt(id),
        user.userId
      );
      if (!materi) {
  ...
  .post(
    "/api/materi",
    requireAuth(async ({ request, user, set }) => {
      try {
  ...
  .put(
    "/api/materi/:id",
    requireAuth(async ({ params: { id }, request, user, set }) => {
      try {
        const formData = await request.formData();
...
export const statsController = new Elysia().get(
  "/api/stats",
  requireAuth(async ({ user }) => {
    try {
